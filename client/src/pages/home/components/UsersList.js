...
function UsersList({ searchKey, socket, onlineUsers }) {
  ...
  const getLastMsg = (userObj) => {
    ...
    if (!chat || !chat.lastMessage) {
      ...
      const lastMsgPerson =
        chat?.lastMessage?.sender === user._id ? "Você: " : "";
      return (
        ...
        return (
          <div
            className={`shadow-sm border p-2 rounded-xl bg-white flex justify-between items-center cursor-pointer w-full
                ${getIsSelctedChatOrNot(userObj) && "border-[#00510f] border-2"}
            `}
            key={userObj._id}
          ...
          >
            ...
            <div onClick={() => createNewChat(userObj._id)}>
              {!allChats.find((chat) =>
                ...
                <button className="border-[#00510f] border text-[#00510f] bg-white p-1 rounded">
                  Criar Chat
                </button>
              )}
            </div>
